...
# Sheet config per shift — identical column layout, different sheet name and Inputs columns
SHIFT_CONFIG = {
    "1st": {
        "sheet":        "Staffing sheet 1ST Shift",
        "inputs_col_name":    "E",   # worker name column in Inputs
        "inputs_col_present": "F",   # present marker column in Inputs
        "usecols":      "A,D,F,H,I", # Name, Skills, BestFit, Present, RecommendedTask
    },
    "2nd": {
        "sheet":        "Staffing Sheet 2nd Shift",
        "inputs_col_name":    "N",   # worker name column in Inputs
        "inputs_col_present": "O",   # present marker column in Inputs
        "usecols":      "A,D,F,H,I", # same layout
    },
}
...
def load_oc_customer_list():
    if not os.path.exists(OC_FILE):
        st.error(f"OC customer list file not found: '{OC_FILE}'. Make sure it is in the same folder as report.py.")
        return []
    try:
        ...
        for row_idx in range(OC_DATA_START, ws.max_row + 1):
            ...
            customers.append({
                "name": name_clean, "aliases": aliases,
                "customer_number": str(raw_cust_num).strip() if raw_cust_num else None,
                "issue": issue, "requirements": reqs,
                "sign_off": sign_off, "pictures": pictures, "priority": priority,
            })
        return customers
...
def find_oc_customers_in_board(board_text):
    ...
    for customer in oc_list:
        ...
        found_terms = [t for t in search_terms if t.lower() in board_lower]
        if found_terms:
...
def calculate_input_values(day, shift, total_cases):
    first_shift_pick  = {"Sunday":0.20,"Monday":0.18,"Tuesday":0.18,"Wednesday":0.19,"Thursday":0.19,"Friday":0.18,"Saturday":0.21}
    second_shift_pick = {"Sunday":0.19,"Monday":0.15,"Tuesday":0.15,"Wednesday":0.17,"Thursday":0.17,"Friday":0.17,"Saturday":0.19}
    first_shift_fp    = {"Sunday":0.28,"Monday":0.32,"Tuesday":0.40,"Wednesday":0.35,"Thursday":0.35,"Friday":0.36,"Saturday":0.31}
    second_shift_fp   = {"Sunday":0.32,"Monday":0.33,"Tuesday":0.27,"Wednesday":0.29,"Thursday":0.28,"Friday":0.30,"Saturday":0.30}
    if shift == "1st":
...
def calculate_needed(day, shift, total_cases, hours_remaining, total_outbound_loads_actual,
                     crossroads_open, deer_creek_open, msb_open):
    if hours_remaining <= 0:
        hours_remaining = 1
    cases_to_pick, full_pallets = calculate_input_values(day, shift, total_cases)
    inbound_pallets = 0
    if crossroads_open == "YES": inbound_pallets += 700
    if deer_creek_open == "YES": inbound_pallets += 500
    if msb_open == "YES":        inbound_pallets += 640
    raw_needed = {
    ...
    needed = {
        ...
        "Tasking":   whole_workers(raw_needed["Putaway"] + raw_needed["Replenishment"] + raw_needed["Full Pallets"]),
...
def generate_recommendations(staff, needed):
    ...
    for idx in present_indexes:
        ...
        if name_contains(row, "Dale"):
            staff.at[idx, "Recommended Task"] = "Receiving"; assigned["Receiving"] += 1
        elif name_contains(row, "Alex"):
            staff.at[idx, "Recommended Task"] = "Unloading"; assigned["Unloading"] += 1

    for idx in present_indexes:
        if staff.at[idx, "Recommended Task"] != "": continue
        row = staff.loc[idx]
        if str(row["Skills"]).strip() == "P":
            assign_if_needed("Picking", idx)

    best_fit_steps = [
        ("Unloading","Unload","U"), ("Loading","Load","L"),
        ("Receiving","Receiv","R"), ("Picking","Pick","P"), ("Tasking","Task","T"),
    ]
    ...
    for task, skill in skill_map.items():
        for idx in present_indexes:
            if assigned[task] >= needed[task]: break
            if staff.at[idx, "Recommended Task"] != "": continue
            if has_skill(staff.loc[idx], skill):
                assign_if_needed(task, idx)
        ...
        while assigned[task] < needed[task]:
            ...
            for idx in present_indexes:
                ...
                if best_fit(row,"Task") and (has_skill(row,"T") or has_skill(row,"L") or has_skill(row,"P")):
                    assign_if_needed(task, idx); found = True; break
            if found: continue
            for idx in present_indexes:
                if staff.at[idx, "Recommended Task"] != "": continue
                row = staff.loc[idx]
    ...
    for idx in present_indexes:
        if staff.at[idx, "Recommended Task"] == "":
            if assigned["Tasking"] < needed["Tasking"]:
                staff.at[idx, "Recommended Task"] = "Tasking"; assigned["Tasking"] += 1
            else:
                staff.at[idx, "Recommended Task"] = "Lead/Extra"

    preferred_extra_names = ["will", "antonio"]
    preferred_idxs = [idx for idx in present_indexes
                      if any(n in str(staff.at[idx,"Name"]).lower() for n in preferred_extra_names)]
    current_extra_idxs = [idx for idx in present_indexes
                          if staff.at[idx,"Recommended Task"] == "Lead/Extra"]
    for preferred_idx in preferred_idxs:
        if not current_extra_idxs: break
        if staff.at[preferred_idx,"Recommended Task"] == "Lead/Extra": continue
        swap_idx = None
        for extra_idx in current_extra_idxs:
            if not any(n in str(staff.at[extra_idx,"Name"]).lower() for n in preferred_extra_names):
                swap_idx = extra_idx; break
...
def build_recommendations(summary_table, present_recommendations, raw_needed, hours_remaining, notes):
    ...
    for task, row in summary_table.iterrows():
        ...
        if diff < 0:
            recommendations.append(f"{task}: approximately {abs(diff * hours_remaining):.1f} labor-hours behind.")
        elif diff > 0:
    ...
    if picking_gap < 0:
        recommendations.append("High picking short risk detected. Consider moving tasking labor into replenishment.")
        recommendations.append("Avoid pulling pickers into unloading or loading unless outbound service is critical.")
        if tasking_gap > 0:
            recommendations.append(f"Tasking has {tasking_gap} extra worker(s). Consider temporarily assigning to replenishment.")
        if lead_gap > 0:
            recommendations.append("Lead/Extra capacity available. Consider flexing into replenishment or picking support.")
    if unloading_gap < 0 or receiving_gap < 0:
        recommendations.append("Inbound flow risk detected. Falling behind may create dock congestion.")
    ...
    if loading_gap < 0:
        recommendations.append("Outbound loading risk detected. Late departures may increase.")
        recommendations.append("Protect loading labor before reallocating.")
        if lead_gap > 0:
    ...
    if inbound_pressure > outbound_pressure * 1.3:
        recommendations.append("Inbound workload significantly heavier than outbound. Focus on unloading, receiving, and putaway.")
    elif outbound_pressure > inbound_pressure * 1.3:
        recommendations.append("Outbound workload significantly heavier than inbound. Prioritize replenishment and picking.")
    ...
    if "late"  in lower_notes: recommendations.append("Manager notes mention late loads. Prioritize outbound execution.")
    if "short" in lower_notes: recommendations.append("Manager notes indicate short risk. Protect replenishment and picking.")
    if "live"  in lower_notes: recommendations.append("Live loads detected. Prioritize those doors before drop trailers.")
    if "cpu"   in lower_notes: recommendations.append("CPU loads referenced. Ensure loading labor is protected.")
...
def normalize_board_text(value):
    ...
    if isinstance(value, datetime.time):     return value.strftime("%H:%M")
    if isinstance(value, datetime.datetime): return value.strftime("%m/%d/%Y")
    try:
        import math
...
def normalize_board_date(value):
    ...
    for fmt in ["%m/%d/%Y","%m/%d/%y","%Y-%m-%d"]:
        try: return pd.to_datetime(text, format=fmt).strftime("%m/%d/%Y")
        except Exception: pass
    try:
        parsed = pd.to_datetime(text, errors="coerce")
        if pd.notna(parsed): return parsed.strftime("%m/%d/%Y")
    except Exception: pass
    return text
...
def normalize_board_time(value):
    ...
    if re.fullmatch(r"\d{1,2}:\d{2}", text):
        h, m = text.split(":"); return f"{int(h):02d}:{m}"
    try:
        parsed = pd.to_datetime(text, errors="coerce")
        if pd.notna(parsed): return parsed.strftime("%H:%M")
    except Exception: pass
    return text
...
def looks_like_board_load(value):
    text = normalize_board_text(value)
    if not text: return ""
    if text in BOARD_DAY_NAMES: return ""
    if re.search(r"\d{1,2}/\d{1,2}/\d{2,4}", text): return ""
    if re.fullmatch(r"\d{1,2}:\d{2}", text): return ""
...
def detect_board_status(value):
    ...
    if not text: return ""
    if "LOADED SHORT" in text:                              return "Loaded Short"
    if "PICKING/SHORT" in text or "PICKING SHORT" in text: return "Picking/Short"
    if "READY/SHORT" in text or re.search(r"\bR/S\b",text):return "R/S"
    if re.search(r"\bRTL\b",text) or "READY TO LOAD" in text: return "RTL"
    if "NO DRIVER" in text:                                return "No Driver"
    if "PICKING" in text:                                  return "Picking"
    if "COMPLETED" in text or re.search(r"\bCOMPLETE\b",text): return "Completed"
    if re.search(r"\bLATE\b",text):                        return "Late"
    if re.search(r"\bLOADED\b",text):                      return "Loaded"
    return ""

def detect_trailer_field_late(trailer_value):
    text = normalize_board_text(trailer_value).upper()
    if not text: return False
    if re.search(r"\bLATE\b", text): return True
    if re.match(r"^ETA\b", text):    return True
    return False
...
def board_cell_flags(cell):
    ...
    try:
        ...
        if fill and fill.fgColor:
            if fill.fgColor.type == "rgb":     fill_color = str(fill.fgColor.rgb).upper()
            elif fill.fgColor.type == "indexed": fill_color = str(fill.fgColor.indexed).upper()
    except Exception: pass
    try:
        font = cell.font
        if font and font.color and font.color.type == "rgb": font_color = str(font.color.rgb).upper()
    except Exception: pass
    if fill_color in ("FFFFFF00","00FFFF00","FFFF00","0000000D"):          flags.append("LOAD-CHECK")
    if fill_color in ("FFADD8E6","FF87CEEB","FFADD8FF","FFB0E0E6","FF00BFFF"): flags.append("TT4-NEEDED")
    if font_color in ("FFFF0000","00FF0000"):                              flags.append("CANADIAN")
    return flags
...
def board_records_from_excel(board_file):
    ...
    if file_name.endswith(".xls"):
        ...
        for sheet_name, df in sheets.items():
            ...
            for idx, row in df.iterrows():
                ...
                while len(values) < 13: values.append("")
                if values[0] in BOARD_DAY_NAMES:
                    current_day = values[0]; current_date = normalize_board_date(values[1]); continue
                load_number = looks_like_board_load(values[0])
                if not load_number: continue
                status = "Late" if detect_trailer_field_late(values[5]) else detect_board_status(values[6])
                if not status: status = detect_board_status(" ".join(values))
                t = values[5].upper()
                type_value = "Live" if "LIVE" in t else ("CPU - Live" if "CPU" in t else ("Drop" if "DROP" in t else ""))
                all_rows.append({"source":sheet_name,"day":current_day,"date":current_date,
                    "load_number":load_number,"customer":values[1],"carrier":values[2],
                    "appt_time":normalize_board_time(values[3]),"door":values[4],"trailer":values[5],
                    "status":status,"type":type_value,"tt4":values[7],"loader":values[8],
                    "comments":values[9],"pulls":parse_number(values[10]),"picks":parse_number(values[11]),
                    "flags":[],"raw_row":" | ".join(v for v in values if v)})
        return all_rows

    wb = load_workbook(board_file, data_only=True)
    outbound_sheet = next((c for c in ["Outbound","outbound","OUTBOUND"] if c in wb.sheetnames), None)
    ...
    for sheet_name in sheets_to_read:
        ...
        for row_idx in range(1, ws.max_row + 1):
            values = []; flags = []; has_content = False
            for col_idx in range(1, 14):
                cell = ws.cell(row_idx, col_idx)
                if cell.value is not None: has_content = True
                values.append(normalize_board_text(cell.value))
                for flag in board_cell_flags(cell): flags.append(flag)
            if not has_content:
                consecutive_empty += 1
                if consecutive_empty >= 15: break
                continue
            consecutive_empty = 0
            if values[0] in BOARD_DAY_NAMES:
                current_day = values[0]; current_date = normalize_board_date(values[1]); continue
            ...
            t = values[5].upper()
            type_value = "Live" if "LIVE" in t else ("CPU - Live" if "CPU" in t else ("Drop" if "DROP" in t else ""))
            all_rows.append({"source":sheet_name,"row_number":row_idx,"day":current_day,"date":current_date,
                "load_number":load_number,"customer":values[1],"carrier":values[2],
                "appt_time":normalize_board_time(values[3]),"door":values[4],"trailer":values[5],
                "status":status,"type":type_value,"tt4":values[7],"loader":values[8],
                "comments":values[9],"pulls":parse_number(values[10]),"picks":parse_number(values[11]),
                "flags":sorted(set(flags)),"raw_row":" | ".join(v for v in values if v)})
    return all_rows
...
def board_records_from_csv(board_file):
    ...
    for idx, row in df.iterrows():
        ...
        t = values[5].upper()
        type_value = "Live" if "LIVE" in t else ("CPU - Live" if "CPU" in t else ("Drop" if "DROP" in t else ""))
        all_rows.append({"source":"CSV Board","day":current_day,"date":current_date,
            "load_number":load_number,"customer":values[1],"carrier":values[2],
            "appt_time":normalize_board_time(values[3]),"door":values[4],"trailer":values[5],
            "status":status,"type":type_value,"tt4":values[7],"loader":values[8],
            "comments":values[9],"pulls":parse_number(values[10]),"picks":parse_number(values[11]),
...
def board_records_from_inbound_sheet(board_file):
    board_file.seek(0)
    try: wb = load_workbook(board_file, data_only=True)
    except Exception: return []
    inbound_sheet = next((c for c in ["Inbound","inbound","INBOUND"] if c in wb.sheetnames), None)
    if not inbound_sheet: return []
    ws = wb[inbound_sheet]
    all_rows = []; current_day = current_date = ""
    for row_idx in range(1, ws.max_row + 1):
        ...
        if col1_title in BOARD_DAY_NAMES:
            current_day = col1_title; current_date = normalize_board_date(ws.cell(row_idx, 2).value); continue
        if col1.lower() in ("load number","load #","load"): continue
        load_number = looks_like_board_load(col1)
        if not load_number: continue
        all_rows.append({"source":inbound_sheet,"day":current_day,"date":current_date,
            "load_number":load_number,"carrier":normalize_board_text(ws.cell(row_idx,2).value),
...
def build_python_inbound_summary(inbound_rows):
    summary = {"loads_read_from_inbound":len(inbound_rows),"loads_by_day":{},"live_loads":0,
               "drop_loads":0,"on_lot":0,"at_door":0,"loads_with_receiver":0,"loads_missing_receiver":0}
    for row in inbound_rows:
        day_key = row.get("day") or "Unknown Day"
        summary["loads_by_day"][day_key] = summary["loads_by_day"].get(day_key, 0) + 1
        t = row.get("type","").upper(); s = row.get("status","").upper()
        if "LIVE"  in t: summary["live_loads"] += 1
        if "DROP"  in t: summary["drop_loads"] += 1
        if "ON LOT" in s: summary["on_lot"] += 1
        if "DOOR"  in s: summary["at_door"] += 1
...
def build_python_board_summary(board_rows):
    summary = {"loads_read_from_board":len(board_rows),"loads_by_day":{},"loads_by_date":{},
        "status_counts":{},"late_loads":0,"rtl_loads":0,"rs_loads":0,"picking_loads":0,
        "picking_short_loads":0,"loaded_short_loads":0,"completed_loads":0,
        "blank_or_not_started_loads":0,"live_loads":0,"drop_loads":0,"cpu_loads":0,
        "tt4_needed_loads":0,"load_check_loads":0,"canadian_loads":0,
        "loads_with_loader_assigned":0,"loads_missing_loader":0,
        "late_load_details":[],"rs_load_details":[],"picking_short_details":[],
        "loaded_short_details":[],"rtl_details":[],"blank_or_not_started_details":[],
        "priority_load_details":[]}
    for row in board_rows:
        day_key  = row.get("day")  or "Unknown Day"
        date_key = row.get("date") or "Unknown Date"
        status   = row.get("status") or "Blank/Not Started"
        su = status.upper(); ru = row.get("raw_row","").upper(); flags = row.get("flags",[])
        summary["loads_by_day"][day_key]   = summary["loads_by_day"].get(day_key, 0) + 1
        summary["loads_by_date"][date_key] = summary["loads_by_date"].get(date_key, 0) + 1
        summary["status_counts"][status]   = summary["status_counts"].get(status, 0) + 1
        if "LATE" in su or "LATE " in f" {ru} ":
            summary["late_loads"] += 1; summary["late_load_details"].append(row)
        if su == "RTL" or "READY TO LOAD" in su:
            summary["rtl_loads"] += 1; summary["rtl_details"].append(row)
        if su in ["R/S","READY/SHORT"] or "R/S" in ru:
            summary["rs_loads"] += 1; summary["rs_load_details"].append(row)
        if su == "PICKING": summary["picking_loads"] += 1
        if "PICKING/SHORT" in su or "PICKING SHORT" in su:
            summary["picking_short_loads"] += 1; summary["picking_short_details"].append(row)
        if "LOADED SHORT" in su:
            summary["loaded_short_loads"] += 1; summary["loaded_short_details"].append(row)
        if "COMPLETED" in su or su == "COMPLETE": summary["completed_loads"] += 1
        if not row.get("status"):
            summary["blank_or_not_started_loads"] += 1; summary["blank_or_not_started_details"].append(row)
        if "LIVE" in ru:
            summary["live_loads"] += 1; summary["priority_load_details"].append(row)
        if "DROP" in ru: summary["drop_loads"] += 1
        if "CPU"  in ru:
            summary["cpu_loads"] += 1; summary["priority_load_details"].append(row)
        if "TT4-NEEDED"  in flags: summary["tt4_needed_loads"] += 1;  summary["priority_load_details"].append(row)
        if "LOAD-CHECK"  in flags: summary["load_check_loads"] += 1;  summary["priority_load_details"].append(row)
        if "CANADIAN"    in flags: summary["canadian_loads"] += 1;    summary["priority_load_details"].append(row)
        if row.get("loader"): summary["loads_with_loader_assigned"] += 1
        else:                 summary["loads_missing_loader"] += 1
    seen = set(); unique_priority = []
    for item in summary["priority_load_details"]:
        key = (item.get("load_number"), item.get("row_number"), item.get("source"))
        if key not in seen: seen.add(key); unique_priority.append(item)
...
def actionable_rows_for_ai(board_rows):
    ...
    for row in board_rows:
        ...
        notable      = status and not is_completed and not is_skip
        if is_completed:
            completed.append({"day":row.get("day",""),"load":row.get("load_number",""),
                "customer":row.get("customer",""),"time":row.get("appt_time",""),"status":status})
        elif notable or bool(flags) or is_blank:
            actionable.append({"day":row.get("day",""),"load":row.get("load_number",""),
                "customer":row.get("customer",""),"time":row.get("appt_time",""),
                "door":row.get("door",""),"trailer":row.get("trailer",""),
                "status":status or "Blank/Not Started","type":row.get("type",""),
                "loader":row.get("loader",""),"flags":flags,"comments":row.get("comments","")})
...
def read_board_file_to_text(board_file):
    ...
    try:
        if file_name.endswith(".csv"):
            board_rows = board_records_from_csv(board_file); inbound_rows = []
        else:
        ...
        payload = {
            ...
            "instructions_for_ai": [
                ...
                "All times use 24-hour clock. Blank status means load not yet started.",
                "Flags: LOAD-CHECK=yellow fill, TT4-NEEDED=blue fill, CANADIAN=red font.",
            ],
        }
        return json.dumps(payload, indent=2, ensure_ascii=False)
    except Exception as e:
        st.error(f"BOARD PARSER ERROR: {e}"); st.exception(e)
        return json.dumps({"error":str(e),"python_verified_outbound_summary":{},
            "python_verified_inbound_summary":{},"actionable_outbound_rows":[]}, indent=2)
...
def analyze_board_with_groq(board_text, day, shift, total_cases, hours_remaining, total_outbound_loads,
                             crossroads_open, deer_creek_open, msb_open, needed, summary_table,
                             cases_to_pick, inbound_pallets, notes, oc_alert_text=None):
    client = get_groq_client()
    if client is None:
        return ("Board analysis could not be completed because GROQ_API_KEY is missing. "
    ...
    try:
        ...
        py_summary = {}; py_inbound = {}; actionable_rows = []; completed_rows = []
    ...
    plants_open = [p for p, s in [("Crossroads",crossroads_open),("Deer Creek",deer_creek_open),("MSB",msb_open)] if s=="YES"]
    ...
    verified_counts = (
        ...
        f"Late:{py_summary.get('late_loads',0)}  RTL:{py_summary.get('rtl_loads',0)}  "
        f"R/S:{py_summary.get('rs_loads',0)}  Picking:{py_summary.get('picking_loads',0)}  "
        ...
        f"Live:{py_summary.get('live_loads',0)}  CPU:{py_summary.get('cpu_loads',0)}  "
        f"Canadian:{py_summary.get('canadian_loads',0)}  TT4:{py_summary.get('tt4_needed_loads',0)}  "
    ...
    verified_inbound = (
        ...
        f"Live:{py_inbound.get('live_loads',0)}  Drop:{py_inbound.get('drop_loads',0)}  "
        f"OnLot:{py_inbound.get('on_lot',0)}  AtDoor:{py_inbound.get('at_door',0)}  "
        f"OnLotOrAtDoor:{py_inbound.get('on_lot',0)+py_inbound.get('at_door',0)}\n"
        f"By day: {ib_day_str}"
    )

    actionable_table = _rows_to_table(actionable_rows,
        ["day","load","customer","time","door","status","type","loader","flags","comments"])
    completed_table  = _rows_to_table(completed_rows, ["day","load","customer","time","status"])

    # ── Shift-aware context and goal ─────────────────────────────────────────
    if shift == "1st":
        shift_context = "1st shift 06:00-16:30 (9.5 hrs). First shift loads ~52% of day's loads."
        shift_goal    = ("Goal: Get ahead early so later appointments are protected. "
                         "Always discuss how decisions set up 2nd shift for success. "
                         "Manufacturing only if it genuinely helps this shift.")
        shift_goal_section = ("- How are we setting up 2nd shift for success? What should 2nd shift "
                               "inherit: RTL loads, picked orders, clear dock? State the handoff target.")
    else:
        shift_context = "2nd shift 16:30-03:00 (approx 10 hrs). Second shift loads ~48% of day's loads."
        shift_goal    = ("Goal: Finish the day clean. Leave 1st shift tomorrow as ahead as possible. "
                         "Clear remaining loads, resolve any shorts, stage product for next morning. "
                         "Manufacturing only if it resolves a short that would hurt 1st shift tomorrow.")
        shift_goal_section = ("- How are we setting up 1st shift tomorrow for success? What should they "
                               "inherit: completed loads, no open shorts, clean dock, staged replenishment? "
                               "State the specific handoff target for end of 2nd shift.")
...
CONTEXT: High-volume grocery DC. {shift_context} 24-hr clock.
...
{shift_goal}
For prioritization: OC customer > appointment time > load type (CPU and live loads have high priority).
Late loads: always mention them, but note there is not much we can do without a driver present.
...
1. BOARD SUMMARY
- Loads by status and day using verified counts above. Do not recount. How many of today's loads have not been started.
...
- Inbound summary: use VERIFIED INBOUND COUNTS only — total, by day, live vs drop, on lot vs at door.
...
def write_board_analysis_to_excel(wb, analysis_text, oc_matches=None):
    ...
    dark_blue="0F5B78"; orange="C55A11"; white="FFFFFF"; light_blue="D9EAF7"; light_orange="FCE4D6"
    thin = Side(style="thin", color="B7B7B7"); border = Border(left=thin,right=thin,top=thin,bottom=thin)
    ...
    if oc_matches:
        ...
        for match in oc_matches:
            c = match["customer"]
            oc_lines = [f"CUSTOMER: {c['name'].upper()}  |  Priority: {c['priority']}",
                        f"Issue History: {c['issue']}", f"DC Requirements: {c['requirements']}"]
            if c["sign_off"]:  oc_lines.append("DC Supervisor Sign-Off REQUIRED before this load ships.")
            if c["pictures"]:  oc_lines.append("Photos REQUIRED: 3 on dock + 3 during loading (6 total). Email to manager.")
            for line in oc_lines:
                ...
                ws.merge_cells(start_row=current_row,start_column=1,end_row=current_row,end_column=7)
                ws.row_dimensions[current_row].height = max(15, min(60, len(line)//5)); current_row += 1
            current_row += 1
        current_row += 1
...
def write_recommendations_to_excel(wb, staff, shift):
    ...
    present_col = cfg["inputs_col_present"]
    ...
    for row in range(2, ws_crew.max_row + 1):
        ...
        if name: crew_name_to_row[str(name).strip().lower()] = row

    for _, row in staff.iterrows():
        name = str(row["Name"]).strip().lower(); task = row["Recommended Task"]
        if name in crew_name_to_row:
            crew_row = crew_name_to_row[name]
            ws_crew[f"C{crew_row}"] = task; ws_crew[f"D{crew_row}"] = task


def build_dashboard(wb, summary_table, present_recommendations, recommendations, shift="1st", oc_matches=None):
    if "Staffing Dashboard" in wb.sheetnames:
        ws_dash = wb["Staffing Dashboard"]; ws_dash.delete_rows(1, ws_dash.max_row)
    else:
        ws_dash = wb.create_sheet("Staffing Dashboard")
    dark_blue="0F5B78"; orange="C55A11"; light_blue="D9EAF7"; green="C6EFCE"
    red="FFC7CE"; yellow="FFEB9C"; white="FFFFFF"
    thin = Side(style="thin",color="B7B7B7"); border = Border(left=thin,right=thin,top=thin,bottom=thin)

    title = f"{shift} Shift Staffing Dashboard"
    ws_dash["A1"] = title
    ...
    kpis = [("Total Present",total_present),("Total Needed",total_needed),
            ("Total Assigned",total_assigned),("Lead/Extra",lead_extra),("Overall Gap",overall_gap)]
    for (label, value), col in zip(kpis, [1,3,5,7,9]):
    ...
    for c, header in enumerate(["Task","Needed","Assigned","Difference","Status"],1):
        cell = ws_dash.cell(header_row,c); cell.value = header
        cell.font = Font(bold=True, color=white); cell.fill = PatternFill("solid", fgColor=dark_blue)
    ...
    for r, (task, row) in enumerate(summary_table.iterrows(), header_row+1):
        for c, value in enumerate([task,int(row["Needed"]),int(row["Assigned"]),int(row["Difference"]),row["Status"]],1):
            cell = ws_dash.cell(r,c); cell.value = value; cell.border = border
            cell.alignment = Alignment(horizontal="center")
            if c == 5:
                cell.fill = PatternFill("solid", fgColor=(green if value=="Good" else red if value=="Understaffed" else yellow))
    ...
    for rec in recommendations:
        ...
        ws_dash.merge_cells(start_row=rec_row,start_column=7,end_row=rec_row,end_column=11); rec_row+=1
    ...
    for c, header in enumerate(["Name","Skills","Best Fit","Recommended Task"],1):
        cell = ws_dash.cell(board_start+1,c); cell.value = header
        cell.font = Font(bold=True, color=white); cell.fill = PatternFill("solid", fgColor=dark_blue)
        cell.border = border; cell.alignment = Alignment(horizontal="center")
    for r, (_, row) in enumerate(present_recommendations.iterrows(), board_start+2):
        for c, value in enumerate([row["Name"],row["Skills"],row["Best Fit"],row["Recommended Task"]],1):
            cell = ws_dash.cell(r,c); cell.value = value; cell.border = border
            cell.alignment = Alignment(horizontal="center")
            if r % 2 == 0: cell.fill = PatternFill("solid", fgColor=light_blue)

    chart_anchor_row = board_start + len(present_recommendations) + 5
    bar = BarChart(); bar.title = "Needed vs Assigned"; bar.y_axis.title = "Workers"; bar.x_axis.title = "Task"
    ...
    bar.height=9; bar.width=15; bar.legend.position="r"; ws_dash.add_chart(bar,f"E{chart_anchor_row}")
    pie = PieChart(); pie.title = "Assigned Labor Distribution"
    ...
    pie.height=9; pie.width=13; pie.legend.position="r"; ws_dash.add_chart(pie,f"I{chart_anchor_row}")
    for col in range(1,12): ws_dash.column_dimensions[get_column_letter(col)].width = 18
    ws_dash.column_dimensions["A"].width = 22
    for col in ["G","H","I","J","K"]: ws_dash.column_dimensions[col].width = 35
    ws_dash.freeze_panes = f"A{header_row}"


def build_email_draft(day, shift, total_cases, hours_remaining, total_outbound_loads_day,
    ...
    staffing_lines = [
        f"- {task}: Need {int(row['Needed'])}, Assigned {int(row['Assigned'])}, Gap {int(row['Difference'])} ({row['Status']})"
        for task, row in summary_table.iterrows()
    ]
    top_recommendations = "\n".join(f"- {rec}" for rec in recommendations[:8])
    oc_email_block = ""
    if oc_matches:
        oc_names = ", ".join(f"{m['customer']['name'].upper()} [{m['customer']['priority']}]" for m in oc_matches)
    ...
    greeting = "Good morning," if shift == "1st" else "Good afternoon,"
    body = f"""
{greeting}
...
# Load names dynamically based on selected shift
...
present_workers = st.sidebar.multiselect(f"Who is present? ({shift} shift)", names)
...
board_file = st.file_uploader(
    "Upload the outbound load board Excel or CSV file", type=["xlsx","xls","csv"],
...
if board_file:
    ...
    with st.expander("Preview: What Python parsed from the board (no AI tokens used)", expanded=False):
        try:
            board_file.seek(0)
            preview_rows = board_records_from_csv(board_file) if board_file.name.lower().endswith(".csv") else board_records_from_excel(board_file)
            if not preview_rows:
                st.warning("No load rows were parsed. Check that the file has day headers and 5-9 digit load numbers in column A.")
            else:
                ...
                if inbound_preview_rows:
                    ...
                    st.markdown("---"); st.markdown("**Inbound**")
                    ...
                    st.dataframe(pd.DataFrame([{
                        "Day":r.get("day",""),"Load #":r.get("load_number",""),"Carrier":r.get("carrier",""),
                        "Time":r.get("appt_time",""),"Type":r.get("type",""),"Trailer":r.get("trailer",""),
                        "Status":r.get("status",""),"Receiver":r.get("receiver",""),
                        "Origin":r.get("origin",""),"Notes":r.get("notes","")}
                        for r in inbound_preview_rows]), use_container_width=True, height=250)

                st.markdown("---"); st.markdown("**Every outbound load row Python extracted from the file:**")
                st.dataframe(pd.DataFrame([{
                    "Day":r.get("day",""),"Date":r.get("date",""),"Load #":r.get("load_number",""),
                    "Customer":r.get("customer",""),"Carrier":r.get("carrier",""),
                    "Time":r.get("appt_time",""),"Door":r.get("door",""),"Trailer":r.get("trailer",""),
                    "Status":r.get("status","") or "—","Type":r.get("type",""),"TT4":r.get("tt4",""),
                    "Loader":r.get("loader",""),"Picks":r.get("picks",0),"Pulls":r.get("pulls",0),
                    "Flags":", ".join(r.get("flags",[])),"Comments":r.get("comments","")}
                    for r in preview_rows]), use_container_width=True, height=400)
                ...
                if blank_time:     issues.append(f"{len(blank_time)} load(s) have no time: {', '.join(blank_time[:5])}")
                if blank_customer: issues.append(f"{len(blank_customer)} load(s) have no customer: {', '.join(blank_customer[:5])}")
                if no_day:         issues.append(f"{len(no_day)} load(s) have no day context: {', '.join(no_day[:5])}")
                for issue in issues: st.warning(issue)
                if not issues: st.success("All loads have time, customer, and day context — parse looks clean.")
        except Exception as e:
            st.error(f"Preview failed: {e}"); st.exception(e)

with st.expander("View Opportunity Customer List (from Excel file)"):
    oc_list_preview = load_oc_customer_list()
    if oc_list_preview:
        st.dataframe(pd.DataFrame([{
            "Customer":c["name"].title(),"Customer #":c["customer_number"] or "—","Priority":c["priority"],
            "Issue":c["issue"],"DC Requirements":c["requirements"],
            "Sign-Off Required":"Yes" if c["sign_off"] else "No",
            "Photos Required":"Yes" if c["pictures"] else "No"} for c in oc_list_preview]),
...
if st.button("Generate Staffing Report"):
    ...
    total_outbound_loads_actual = total_outbound_loads_day * (0.52 if shift == "1st" else 0.48)

    ws["B1"] = day; ws["B2"] = shift; ws["B3"] = total_cases
    ws["B4"] = hours_remaining; ws["B8"] = crossroads_open
    ws["B9"] = deer_creek_open; ws["B10"] = msb_open

    cases_to_pick, full_pallets = calculate_input_values(day, shift, total_cases)
    ws["B5"] = cases_to_pick; ws["B6"] = full_pallets; ws["B7"] = total_outbound_loads_actual

    # Mark present workers in the correct Inputs column for this shift
    name_col    = cfg["inputs_col_name"]
    present_col = cfg["inputs_col_present"]
    _last_name_row = 3
    for _r in range(3, ws.max_row + 1):
        if ws[f"{name_col}{_r}"].value and str(ws[f"{name_col}{_r}"].value).strip():
            _last_name_row = _r
        elif _r > _last_name_row + 10:
            break
    for row in range(3, _last_name_row + 1):
        ws[f"{present_col}{row}"] = ""
    selected = {n.strip().lower() for n in present_workers}
    for row in range(3, _last_name_row + 1):
        worker_name = ws[f"{name_col}{row}"].value
        if worker_name and str(worker_name).strip().lower() in selected:
            ws[f"{present_col}{row}"] = "x"
    ...
    # Read the correct shift sheet — same columns (A,D,F,H,I) for both shifts
    staff = pd.read_excel(working_file, sheet_name=cfg["sheet"], usecols="A,D,F,H,I")
    ...
    board_analysis_text = None; oc_matches = []
    ...
    if board_file is not None:
        with st.spinner("Reading board file → scanning for Opportunity Customers → running AI analysis..."):
            ...
            if oc_matches:
                st.warning(f"**Opportunity Customer Alert:** "
                           f"**{', '.join(m['customer']['name'].upper() for m in oc_matches)}** detected on today's board. "
    ...
    build_dashboard(wb, summary_table, present_recommendations, recommendations, shift=shift, oc_matches=oc_matches)
    ...
    if oc_matches:
        st.markdown("---"); st.subheader("Opportunity Customer Alerts")
        st.error("The following customers on today's board are on the Opportunity Customer List and require special DC actions before their loads ship.")
        for match in oc_matches:
            c = match["customer"]
            with st.expander(f"{c['name'].upper()}  —  Priority: {c['priority']}", expanded=True):
                ...
                if c["sign_off"]: st.markdown("**DC Supervisor Sign-Off REQUIRED before this load ships.**")
                if c["pictures"]: st.markdown("**Photos REQUIRED:** 3 on dock + 3 during loading (6 total). Email to manager.")
    ...
    st.dataframe(present_recommendations[["Name","Skills","Best Fit","Recommended Task"]].reset_index(drop=True), use_container_width=True)
    st.subheader("Written Recommendations / What-Ifs")
    for rec in recommendations: st.write(f"• {rec}")

    if board_analysis_text:
        st.markdown("---"); st.subheader("Board Excel Analysis — AI Insights")
        st.info("The analysis below was generated by Groq AI reading the board Excel/CSV file directly from cell values, including color flags for load checks, TT4s, and Canadian loads.")
        st.markdown(board_analysis_text)

    st.download_button(label="Download Staffing Report", data=output,
        file_name=f"Staffing Report {shift} Shift Generated.xlsx",
    ...
    st.markdown("---"); st.subheader("Email Ready to Send")
